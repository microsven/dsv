...
static int try_merge_1 (register struct decision *old,struct decision *add);
static int no_same_mode (struct decision *node);
static int same_modes ( register struct decision *p, register enum machine_mode mode);
static int same_codes ( register struct decision *p, register RTX_CODE code);
static void clear_modes ( register struct decision *p);
static void clear_codes(register struct decision *p);
static void change_state ( char *oldpos, char *newpos);

struct decision *make_sequence(rtx pattern)
{
	register int i;
	rtx x;
...
struct decision * add_to_sequence ( rtx pattern, struct decision *last, char *position)
{
	RTX_CODE code;
	struct decision *new = (struct decision *) malloc (sizeof (struct decision));
...
struct decision * merge_trees (struct decision *old, struct decision *add)
{
	while (add) {
...
{
	while (old) {
		if ((old->position == add->position || (old->position && add->position && !strcmp (old->position, add->position)))
			&& (old->tests == add->tests || (old->tests && add->tests && !strcmp (old->tests, add->tests)))
			&& (old->c_test == add->c_test || (old->c_test && add->c_test && !strcmp (old->c_test, add->c_test)))
			...
			&& old->dupno == add->dupno
			&& old->opno == add->opno
			&& (old->tests == 0 || (add->enforce_mode ? no_same_mode (old) : old->next == 0))
			&& old->code == add->code
			&& old->mode == add->mode) {

			old->success = merge_trees (old->success, add->success);
			assert(!(old->insn_code_number >= 0 && add->insn_code_number >= 0));
...
struct decision * try_merge_2 ( struct decision *old, struct decision *add)
{
	...
	while (p) {
		if (p->position == add->position 
			|| (p->position && add->position && !strcmp (p->position, add->position))) {
			if (!operand && p->tests)
				break;
			if (prev_win && prev_win->enforce_mode && prev_win->mode == add->mode && p->mode != add->mode) {
...
void write_tree ( struct decision *tree, char *prevpos, int afterward, char *afterpos)
{
	...
	/* Find the next alternative to p
	 ...
	 consider all modes mutually exclusive.	*/
		...
		assert(! (p->success == 0 && p->insn_code_number < 0));
	...
	if (in_switch == NO_SWITCH && pos[depth-1] != '*') {
		...
		for (p1 = p, i = 0; p1 && p1->mode != VOIDmode && p1->tests == 0; p1 = p1->next, i++) {
	...
	if (p->mode && !ignmode && in_switch != MODE_SWITCH)
		printf ("GET_MODE (x%d) == %smode && ", depth, GET_MODE_NAME (p->mode));
	...
	if (p->opno >= 0)
		printf (")\n	{ recog_operand[%d] = x%d; ", p->opno, depth);
	else
		printf (")\n	");
...
char * concat ( char *s1, char *s2)
{
	...
	tem = (char *) malloc (strlen (s1) + strlen (s2) + 2);
	strcpy (tem, s1);
	strcat (tem, " ");
	strcat (tem, s2);
